    ...
    try:
        ...
        for line in file:
            ...
            if flag == 0:  # Storing Instruction in instruction_memory
                ...
                instruction_memory["0x"+'0'*(8-(len(hex(mem_location))-2))+hex(mem_location)[2:]] = instruction[2:4]
                instruction_memory["0x"+'0'*(8-(len(hex(mem_location + 1))-2))+hex(mem_location + 1)[2:]] = instruction[4:6]
                instruction_memory["0x"+'0'*(8-(len(hex(mem_location + 2))-2))+hex(mem_location + 2)[2:]] = instruction[6:8]
                instruction_memory["0x"+'0'*(8-(len(hex(mem_location + 3))-2))+hex(mem_location + 3)[2:]] = instruction[8:10]
            if tmp[1] == '$':
                flag = 1
            elif flag == 1:  # Storing Data in Data_memory
                mem_location = int(address[2:], 16)
                data_memory["0x"+'0'*(8-(len(hex(mem_location))-2))+hex(mem_location)[2:]] = instruction[2:4]
                data_memory["0x"+'0'*(8-(len(hex(mem_location + 1))-2))+hex(mem_location + 1)[2:]] = instruction[4:6]
                data_memory["0x"+'0'*(8-(len(hex(mem_location + 2))-2))+hex(mem_location + 2)[2:]] = instruction[6:8]
                data_memory["0x"+'0'*(8-(len(hex(mem_location + 3))-2))+hex(mem_location + 3)[2:]] = instruction[8:10]
        file.close()
    except:
...
def fetch():
    ...
    IR = '0x'+instruction_memory[hex(pc)]+instruction_memory[hex(
        pc + 1)] + instruction_memory[hex(pc+2)]+instruction_memory[hex(pc+3)]
